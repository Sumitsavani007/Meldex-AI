...
function coerceWorkspaceAgentResponse(value: unknown, depth = 0): WorkspaceAgentResponse {
  if (Array.isArray(value)) return coerceWorkspaceAgentResponse({ files: value }, depth);
  const raw = (value || {}) as {
    plan?: unknown;
    files?: unknown;
    commands?: unknown;
  ...
  let files: WorkspaceFileAction[] = Array.isArray(raw.files) ? raw.files.map((item) => {
  ...
  if (!files.length) {
    ...
    const html = record["index.html"] ?? raw.indexHtml ?? raw.html;
    const css = record["style.css"] ?? raw.styleCss ?? raw.css;
    const js = record["script.js"] ?? raw.scriptJs ?? raw.js ?? raw.javascript;
    const recovered: Array<WorkspaceFileAction | null> = [
      typeof html === "string" ? { operation: "create" as const, path: "index.html", content: decodeGeneratedContent(html), description: "HTML generated by model" } : null,
      typeof css === "string" ? { operation: "create" as const, path: "style.css", content: decodeGeneratedContent(css), description: "CSS generated by model" } : null,
      typeof js === "string" ? { operation: "create" as const, path: "script.js", content: decodeGeneratedContent(js), description: "JavaScript generated by model" } : null,
    ];
    files = recovered.filter((file): file is WorkspaceFileAction => Boolean(file?.path));
  }
...
function parseAgentJson(raw: string): WorkspaceAgentResponse {
  ...
  try {
    const parsed = coerceWorkspaceAgentResponse(JSON.parse(candidate));
